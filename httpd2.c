...
static void do_file_responce(struct HTTPRequest *req, FILE *out, char *docroot);
...
respond_to(struct HTTPRequest *req, FILE *out, char *docroot)
{
  if(strcmp(req->method, "GET") == 0)
    do_file_responce(req, out, docroot);
...
static void
do_file_responce(struct HTTPRequest *req, FILE *out, char *docroot)
{
...
get_fileinfo(char *docroot, char *urlpath)
{
  ...
  info = xmalloc(sizeof(struct FileInfo)); info->path = build_fspath(docroot, urlpath);
  info->ok = 0;
